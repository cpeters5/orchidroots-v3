...
def orchid_home(request):
    ...
    for e in rangenushyb_list:
        if len(randimages) > 21:
            break
        if HybImages.objects.filter(gen=e):
            randimages.append(HybImages.objects.filter(gen=e).filter(rank__gt=0).filter(rank__lt=9).order_by('?')[0:1][0])
    ...
    context = {'title': 'orchid_home', 'num_visits':num_visits,
               'randimages':randimages,'genus_list':genus_list,
